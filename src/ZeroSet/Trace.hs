...
data ExplorationTree v e = Root v [RoseTree.Tree (e :+ v)] deriving (Show,Eq)
...
exploreWith                :: forall p r. (Ord r, Fractional r, Show r, Show p)
                           ...
                           -> p :+ Cell r -- ^ start
                           -> [p :+ Cell r] -- ^ all cells
                           -> ExplorationTree (p :+ Cell r) CardinalDirection
...
explorePathWith          :: (Ord r, Fractional r, Show r, Show p)
                         => ((p :+ Cell r) -> Bool) -- ^ continue exploring?
                         -> (p :+ Cell r) -> [p :+ Cell r]
                         -> Alternating (p :+ Cell r) CardinalDirection
...
withCorners                    :: forall r sign. (Fractional r, Ord r)
                               => Alternating (Signs sign :+ Cell r) CardinalDirection
                               -> Alternating (Signs sign :+ Cell r) (LineSegment 2 sign r)
withCorners (Alternating v xs) = Alternating v $ map f xs
  where
    f (d :+ z@(ss :+ c)) = s :+ z
      where
...
withEdges             :: Fractional r
                      => Alternating (v :+ Cell r) CardinalDirection
